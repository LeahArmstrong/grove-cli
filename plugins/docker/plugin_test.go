...
func TestExternalStrategy_RelativeWorktreePath(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			absPath: "/home/dev/compose-dev/admin-feature-x",
			want:    "./admin-feature-x",
		},
		{
			name:    "subdirectory",
			absPath: "/home/dev/compose-dev/admin",
			want:    "./admin",
		},
	}
...
func TestExternalStrategy_EnvForWorktree(t *testing.T) {
	...
	env := s.envForWorktree("/home/dev/compose-dev/admin-feature-x")
	if len(env) != 1 {
